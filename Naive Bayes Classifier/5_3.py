...
for f in F:
    model = nbc(f)
    training_set_full = pd.read_csv('./trainingSet.csv')
    training_data = training_set_full.sample(frac=f, random_state = 47)
    training_data.reset_index(drop = True, inplace = True)
    
    training_accuracy = Prediction(training_data, model)
    training_F.append(training_accuracy)

    test_set = pd.read_csv('./testSet.csv')
    test_accuracy = Prediction(test_set, model)
    test_F.append(test_accuracy)
    
plt.plot(F, training_F, label = "Training Accuracy")
plt.plot(F, test_F, label = "Test Accuracy")
