...
data = data.sample(frac = 0.5, random_state = 32)
...
def train_test_split (i, S):
    test_set = S[i]
    Sc = S[:i] + S[(i+1):]
    train_set = pd.concat(Sc, ignore_index = True)
...
for d in d_list:
    ...
    for i in range(len(test_acc_lists)):
        ...
        results_one_d[methods[i]] = [mean, se, test_acc_lists[i]]
    
    # Store in dict for all t_frac values
    results_all_d[d] = results_one_d


# In[34]:


results_df = pd.DataFrame.from_dict(results_all_d, orient='index')
...
results_df.to_csv('./results_df_depth.csv', index = False)
# results_df = pd.read_csv('./results_df_depth.csv')
...
plt.errorbar(d_list, y1, yerr=yerr1, label='Decision Tree')
plt.errorbar(d_list, y2, yerr=yerr2, label='Bagged Trees')
plt.errorbar(d_list, y3, yerr=yerr3, label='Random Forests')
...
'''Hypothesis Test'''
df = results_df[['DT_mean', 'DT_se', 'RF_mean', 'RF_se']]


# In[23]:


df['t_value'] = abs((df['DT_mean']-df['RF_mean'])/(df['DT_se']-df['RF_se']))


# In[24]:


df['df'] = (9*(df['DT_se'].pow(2)+df['RF_se'].pow(2)).pow(2))/(df['DT_se'].pow(4)+df['RF_se'].pow(4))
...
'''
critical t-value = 2.1 for df = 17 or 18 --> reject H0 for all 
'''
