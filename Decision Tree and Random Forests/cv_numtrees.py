...
def bagging(trainingSet, testSet, d, num_trees): 
    tree = {}
...
def randomForests(trainingSet, testSet, d, num_trees):
    tree = {}
...
for t in t_list:
    ...
    for i in range(len(S)):
        ...
        _, test_BT = bagging(train_set, test_set, d, t)
        _, test_RF = randomForests(train_set, test_set, d, t)
        
        # Append to list to obtain list of 10 accuracy values
        test_BT_list.append(test_BT)
        test_RF_list.append(test_RF)
    
    test_acc_lists = [test_BT_list, test_RF_list] 
    methods = ['BT', 'RF']
    ...
    for i in range(len(test_acc_lists)):
        ...
        results_one_t[methods[i]] = [mean, se, test_acc_lists[i]]
    
    # Store in dict for all t_frac values
    results_all_t[t] = results_one_t


# In[17]:


results_df = pd.DataFrame.from_dict(results_all_t, orient='index')
...
results_df = results_df.drop(columns = ['BT', 'RF','BT_acc', 'RF_acc'])
results_df
...
results_df.to_csv('./results_df_numtrees.csv', index = False)
# results_df = pd.read_csv('./results_df_numtrees.csv')
